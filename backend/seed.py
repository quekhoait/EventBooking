...
def seed_more_events():
    ...
    try:
        ...
        locations = LocationModel.query.filter(
            LocationModel.parent_id.isnot(None)
        ).all()
...
# ----------------------------------------------------------------------
def clear_data():
    """Xóa sạch toàn bộ dữ liệu tất cả các bảng và reset AUTO_INCREMENT."""
...
def seed_users():
    ...
    try:
        ...
        db.session.commit()
        print(f"✅ Tạo User thành công với {len(users)} tài khoản mẫu!")
    except Exception as e:
        db.session.rollback()
...
def seed_events_and_details():
    ...
    try:
        ...
        ticket_type_std = EventTicketType.query.filter_by(
            name="Vé Thường (Standard)"
        ).first()
        ...
        if ticket_type_vip:
            print(
                f"• seat_type_id  : {ticket_type_vip.id} ({ticket_type_vip.name} - 500k)"
            )
        if ticket_type_std:
            print(
                f"• seat_type_id  : {ticket_type_std.id} ({ticket_type_std.name} - 200k)"
            )
