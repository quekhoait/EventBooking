...
class EventSeatCreateSchema(BaseSchema):
    event_ticket_type_id = fields.Integer(
        required=True, error_messages={"required": "ID loại vé không được để trống."}
    )
...
class BaseEventSchema(BaseSchema, DateRangeValidationMixin):
    ...
    name = fields.String(
        ...
        validate=validate.Length(
            min=5, max=100, error="Tên sự kiện phải từ 5 đến 100 ký tự."
        ),
    ...
    image = fields.Url(
        allow_none=True,
        required=False,
        error_messages={"invalid": "Đường dẫn ảnh không hợp lệ."},
    )
    ...
    max_per_user = fields.Integer(
        ...
        validate=validate.Range(
            min=1, error="Số vé tối đa mỗi người phải từ 1 trở lên."
        ),
    )
    ...
    event_seats = fields.Nested(
        EventSeatCreateSchema, many=True, allow_none=True, required=False
    )
...
class EventPublishSchema(BaseEventSchema):
    image = fields.Url(
        required=True, error_messages={"required": "Đường dẫn ảnh không được để trống."}
    )
    location_id = fields.Integer(
        required=True, error_messages={"required": "ID địa điểm không được để trống."}
    )
    category_id = fields.Integer(
        required=True, error_messages={"required": "ID danh mục không được để trống."}
    )
    creator_id = fields.Integer(
        required=True,
        error_messages={"required": "ID người tạo sự kiện không được để trống."},
    )

    start_time = fields.DateTime(
        required=True,
        error_messages={"required": "Thời gian mở bán không được để trống."},
    )
    end_time = fields.DateTime(
        required=True,
        error_messages={"required": "Thời gian kết thúc bán vé không được để trống."},
    )
    event_start_time = fields.DateTime(
        required=True,
        error_messages={"required": "Thời gian bắt đầu sự kiện không được để trống."},
    )
    event_end_time = fields.DateTime(
        required=True,
        error_messages={"required": "Thời gian kết thúc sự kiện không được để trống."},
    )
    ...
    event_seats = fields.Nested(
        ...
        required=True,
        validate=validate.Length(
            min=1, error="Sự kiện phải có ít nhất 1 loại vé/ghế khi xuất bản."
        ),
...
class EventDetailResponseSchema(BaseSchema):
    ...
    event_seats = fields.Nested(
        EventSeatResponseSchema, many=True, dump_only=True, attribute="seats"
    )
...
class EventFilterQuerySchema(BaseSchema):
    page = fields.Integer(
        load_default=1, validate=validate.Range(min=1, error="Trang phải lớn hơn 0.")
    )
    page_size = fields.Integer(
        load_default=10,
        validate=validate.Range(min=1, max=100, error="Kích thước trang từ 1 đến 100."),
    )
