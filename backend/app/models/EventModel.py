...
class EventModel(SoftDeleteModel):
    ...
    category_id = db.Column(
        db.Integer, db.ForeignKey("event_category.id"), nullable=True
    )
    ...
    company = db.relationship(
        "Company", backref="events", lazy=True
    )  # <-- THÊM DÒNG NÀY
...
class EventSeat(BaseModel):
    ...
    event_ticket_type_id = db.Column(
        db.Integer, db.ForeignKey("event_ticket_type.id"), nullable=False
    )
